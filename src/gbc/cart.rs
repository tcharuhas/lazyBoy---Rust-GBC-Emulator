...
impl Cart {
    ...
    fn get_mbc_info(bytes: &Box<[u8]>) -> MbcInfo {
        ...
        match bytes[0x0147] {
            ...
            _ => panic!("Unsupported mbc_info", bytes[0x0147]),
        }
    }
    ...
    pub fn rom_size(&self) -> u32 {
        match self.bytes[0x0148] {
            ...
            5 => 1024 * 1024,
            6 => 1024 * 1024 * 2,
            _ => panic!("Unsupported rom size", self.bytes[0x0148]),
        }
    }
    ...
    fn get_ram_size(bytes: &Box<[u8]>) -> u32 {
        match bytes [0x149] {
            ...
            3 => 1024 * 32,
            4 => 1024 * 128,
            _ => panic!("Unsupported ram size", bytes[0x0149]),
        }
    }
    ...
    pub fn gameboy_type(&self) -> GameboyType {
        match self.bytes[0x0143] {
            //need to confirm this is correct at some point
