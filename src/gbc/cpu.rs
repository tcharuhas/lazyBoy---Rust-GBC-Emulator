...
impl Dst<u8> for ZMem<Reg8> {
    fn wrie(self, cpu: &mut Cpu, val: u8) {
        let ZMem(reg) = self;
        let offset = reg.read(cpu) as u16;
        let addr = 0xff00 + offset;
        cpu.interconnect.write(addr, val)
...
impl Cpu {
    ...
    fn handle_interrupt(&mut self) -> u32 {
        let ints = self.interconnect.int_flags & self.interconnecti.nt_enabl;elapsed_cycles
    ...
    fn execute_instruction(&mut self) -> u32 {
        ...
        let timing = {
            match opcode {
                ...
                _ => panic!("Invalid opcode", opcode, self.reg),
            }
        };
    ...
    fn execute_cb_instruction(&mut self) -> Timing {
        ...
        match opcode {
            ...
            _ => panic!("Cb opcode out of range:", opcode, self.reg),
        };
    ...
    fn ld<T, D: Dst<T>, S: Src<T>>(&mut elf, dst: D, src: S) -> Timin g{
        let value = src.read(self);
        dst.write(self, value);
        Timing::Default
    }

    fn ldi<T, D: Dst<T>, S: Src<T>>(&mut self, dst: D, src: S. inc: Rge16) -> Timing {
